...
public class InterfaceGrafica extends JComponent{
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(int i = 1; i < this.sl.getArena().getAltura(); i++) {
            for(int j = 1; j < this.sl.getArena().getLargura(); j++) {
                p = new Ponto(i, j);

                g2d.setColor(Color.WHITE);
                this.dimensao = 1;
                this.topLeftCorner = this.sl.getArena().getVerticesFromCentroid(p).get(0);
